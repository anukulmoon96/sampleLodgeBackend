...
module.exports.users = (req, res) => {
	let sqlQuery = "select id,name,email,access,last_login from GWR.users where show_on_dashboard = 'yes'";
...
from GWR.Footfall
where date between (?) and (?) and time between '08:30:00' and '22:30:00' and location in (?) and outlet = ?
group by date, location, substr(time,1,2)
order by date asc, time asc;`;

            let data0 = connection.query(sqlQuery, [startDate,endDate, location, outlet])
          let finaldata = [];
...
module.exports.alertslinegraph = (req, res) => {
	...
	if (validator.isDate(startDate) && validator.isDate(endDate)) {
		...
		dates2.map((item)=>{
			...
			let data = connection.query(sqlQuery, [item, location]);
